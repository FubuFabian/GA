...
% archivos para xyg_opt***: 15-02, 15-03, 16-03, 16-04
...
%    keyboard
    ...
    nomarchi=strcat('prueba_v5x5_xyg_60car_binv',nomarchi, '_r', int2str(reso),'.txt');
						...
						for bin=0:beta:255 % aqui es hasta 255, 1 es nomas pa'probar
			   				for x=round(x0-raiz_tres_alfa):round(x0+raiz_tres_alfa)
			      				for y=round(y0-raiz_tres_alfa): round(y0+raiz_tres_alfa)
                                    ...
                                    %Z(y,x)=exp(-((x-x0)^2+(y-y0)^2)/dos_al
                                    %fa_2); %esta linea tabula la exp para
                                    %graficacion
                                end            
                            end 
                        end % for bin..   
