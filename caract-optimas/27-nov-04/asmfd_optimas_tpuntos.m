...
CAROPT(16)=0; %pa'que no se salgan los perfiles de la imagen
%************************************************************************
...
for iteracion=1:30
   ...
   [dX, dY]=busqueda_optimas_tpuntos2(XYTI, MGI, MDxGI, MDyGI, MDxxGI, MDyyGI, MDxyGI, CAROPT, MUE_ENTRE, IO, renentre)
   figure
   subplot(1,2,1)
   plot(dX)
   subplot(1,2,2)
...
%  hold off
   ...
   XYc=repmat([tx ty],[ren,1]);
   T=[s*cos(th) -s*sin(th); s*sin(th) s*cos(th)];
   DespliegaSalvaParcial(XYT, XYc, IMA, iteracion);
end % for iteracion
