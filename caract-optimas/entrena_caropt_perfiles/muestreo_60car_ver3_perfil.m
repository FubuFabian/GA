...
for imagen=1:ren % repetimos el ciclo para todas las imagenes
   ...
   M=dlmread(ncontorno, ' ');
	NoP=M(1,1);% UNIX es (1,1); DOS es (1,2)
	ang_ejem=M(2,1);
   ...
   nomarchi=strcat('perf21_60car_',nomarchi,'.txt');
   		for punto=1:no_puntos-1 % para todos los puntos del contorno
           	...
           	for k=INFLMT:SUPLMT % para el perfil de longitud SUPLMT-INFLMT
                 ...
                 for escala=1:4  
                      	dosalfa_2=2*alfa^2;
                   		dosbeta_2=2*beta^2;
                   		inv_dospialfa_2=1/(2*pi*alfa^2);
                      	espaciamiento=2^(escala-1);
                 			A=MGI(:,:,escala);
   		                	x0=round(X(punto)+dx*espaciamiento);% (x0, y0) origen de la gaussiana de apertura
							y0=round(Y(punto)+dy*espaciamiento);
                  	    if escala==1 in_out=IBW(y0, x0); end % etiqueta dentro (1) fuera (0) del pixel (y0, x0)
                        %I(y0,x0)=255;
                        %*******************        
                        ...
                        H=zeros(1,256);
							for bin=0:255 % aqui es hasta 255, 1 es nomas pa'probar
			   					for x=round(x0-3*sqrt(alfa)):round(x0+3*sqrt(alfa))
                              		...
                              		H(bin+1)=H(bin+1)+exp(-((x-x0)^2+(y-y0)^2)/dosalfa_2)*exp(-((A(y,x)-bin)^2)/dosbeta_2);
                         		end            
                        	end 
                     		H(bin+1)=H(bin+1)*inv_dospialfa_2;   
             				end % for bin..   
             				% plot(H(no_histo,:));
                 ...
                 fid=fopen(nomarchi,'a');
                 fprintf(fid,'punto %d del perfil (1)in_(0)out:\n', k-INFLMT+1);
                 fprintf(fid,'%d\n', double(in_out));
