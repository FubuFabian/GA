% knn_SFS1_perfil_22_perfilentre.m
% 20-dic-2004: este programa utiiza perfiles de entrenamiento de 21 pixeles en lugar de ventanas
...
CAROPT=load('car_opt_sX1_1504.txt');
   ...
   for punto=1:no_puntos-1 % para todos los puntos del contorno
           ...
           for k=INFLMT:SUPLMT % para el perfil de longitud SUPLMT-INFLMT
               ...
               x0=round(X(punto)+dx);% (x0, y0) coord. del pixel
 				 y0=round(Y(punto)+dy);
               %in_out=knn_optimas(MUE_ENTRE, punto, VECTOROP(k-INFLMT+1,:), CAROPTPUNTO, IO, renentre);
               I(y0,x0)=IN_OUT(k-INFLMT+1);
               %I(y0, x0)=in_out;
           end % para un perfil   
           % keyboard
           %imshow(I)
        imwrite(I, '1504_seg_perfilentre.tif', 'tif');  
