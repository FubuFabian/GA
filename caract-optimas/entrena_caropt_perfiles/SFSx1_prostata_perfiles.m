% SFSx1_prostata_perfiles.m
% SFS con archivos de car. muestreadas de perfiles de 21 pixeles
...
for imagen=1:renentre % repetimos el ciclo para todos los archivos
   ...
   name=name(4:11);
   name=strcat('perf21_60car_', name, '.txt');
   fid=fopen(name, 'r');
	for j=1:35
		L=fscanf(fid, '%c',20);
		LN=fscanf(fid, '%d\n',1);% estas 2 lineas leen "punto de referencia n"
			for i=1:21
				P=fscanf(fid, '%c',6);
				PN=fscanf(fid, '%d\n',1); % numero de pixel en el perfil
				P=fscanf(fid, '%c\n',22);
             	IO((j-1)*21+i, imagen)=fscanf(fid, '%d\n',1); % etiqueta I/O
...
for punto=1:35
   ...
   for x=1:60
      ...
      for imagenval=1:renval %1ra mitad de imagenes pa' validacion
         for pixelval=1:21 % para el perfil de validacion
            contadorD=1;
            v1=MUE_ENTRE((punto-1)*21+pixelval,CAROPT,imagenval);
            ...
            for pixelentre=1:21      
                for imagenentre=renval+1:rentotal %2da mitad de imagenes pa' entrenamiento KNN
                    v2=MUE_ENTRE((punto-1)*21+pixelentre,CAROPT,imagenentre);
                 		D(contadorD)=sum((v1-v2).^2);
                 		IOPIXEL(contadorD)=IO((punto-1)*21+pixelentre,imagenentre);
          ...
          error=error+(IO((punto-1)*21+pixelval,imagenval)-IOKNN)^2;
        end %for pixelval=1:..
     end %for imagenval=1:..
     if error<e_min
         e_min=error;
     ...
     error=0;
      for imagenval=1:renval %1ra mitad de imagenes pa' validacion
         for pixelval=1:21 % para la ventana de validacion
            contadorD=1;
            v1=MUE_ENTRE((punto-1)*21+pixelval,CAROPT(2:L),imagenval);
            %calcular las distancias de cada pixel de validacion a la muestra de entrenamiento	              
            for pixelentre=1:21      
                for imagenentre=renval+1:rentotal %2da mitad de imagenes pa' entrenamiento KNN
                    v2=MUE_ENTRE((punto-1)*21+pixelentre,CAROPT(2:L),imagenentre);
                 		D(contadorD)=sum((v1-v2).^2);
                 		IOPIXEL(contadorD)=IO((punto-1)*21+pixelentre,imagenentre);
                 		contadorD=contadorD+1;
                end    
     ...
     if error<=e_min
        CAROPT_TODAS(punto,1:length(CAROPT)-1)=CAROPT(2:L);
     else
        CAROPT_TODAS(punto,1:length(CAROPT))=CAROPT;
     end
     ERRORESx1(punto)=error;
     punto
end % for punto=1:..


save 'car_opt_1504.txt' CAROPT -ASCII

save 'car_opt_sX1_1504.txt' CAROPT_TODAS -ASCII

save 'errores_60car_1504.txt' ERRORES -ASCII

save 'errores_60car_sX1_1504.txt' ERRORESx1 -ASCII
