...
for i=1:ren % repetimos el ciclo para todas las imagenes
   ...
   no_puntos=renC+1;
            ...
            for j=1:ancho_v % para la ventana de ancho_v x alto_v
                  ...
                  VECTOR((j-1)*5+i, 1:12)=MOMENTOS(1,:);
                  VECTOR((j-1)*5+i, 13:24)=MOMENTOS(2,:);
                  VECTOR((j-1)*5+i, 25:36)=MOMENTOS(3,:);
                  VECTOR((j-1)*5+i, 37:48)=MOMENTOS(4,:);
                  VECTOR((j-1)*5+i, 49:60)=MOMENTOS(5,:);
              end % for i..
      		end % for j..
