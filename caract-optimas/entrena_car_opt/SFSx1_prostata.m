...
for punto=1:35
   ...
   for x=1:60
      ...
      for imagenval=1:renval %1ra mitad de imagenes pa' validacion
         for pixelval=1:25 % para la ventana de validacion
            contadorD=1;
            v1=MUE_ENTRE((punto-1)*25+pixelval,CAROPT,imagenval);
            ...
            for pixelentre=1:25      
                for imagenentre=renval+1:rentotal %2da mitad de imagenes pa' entrenamiento KNN
                    v2=MUE_ENTRE((punto-1)*25+pixelentre,CAROPT,imagenentre);
                 		D(contadorD)=sum((v1-v2).^2);
                 		IOPIXEL(contadorD)=IO((punto-1)*25+pixelentre,imagenentre);
          ...
          error=error+(IO((punto-1)*25+pixelval,imagenval)-IOKNN)^2;
        end %for pixelval=1:..
     end %for imagenval=1:..
     if error<e_min
         e_min=error;
     ...
     error=0;
      for imagenval=1:renval %1ra mitad de imagenes pa' validacion
         for pixelval=1:25 % para la ventana de validacion
            contadorD=1;
            v1=MUE_ENTRE((punto-1)*25+pixelval,CAROPT(2:L),imagenval);
            %calcular las distancias de cada pixel de validacion a la muestra de entrenamiento	              
            for pixelentre=1:25      
                for imagenentre=renval+1:rentotal %2da mitad de imagenes pa' entrenamiento KNN
                    v2=MUE_ENTRE((punto-1)*25+pixelentre,CAROPT(2:L),imagenentre);
                 		D(contadorD)=sum((v1-v2).^2);
                 		IOPIXEL(contadorD)=IO((punto-1)*25+pixelentre,imagenentre);
                 		contadorD=contadorD+1;
                end    
...
save 'car_opt.txt' CAROPT -ASCII

save 'car_opt_sX1.txt' CAROPT_TODAS -ASCII
